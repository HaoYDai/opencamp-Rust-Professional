...
fn get_deserialization_data(content: String) -> Vec<HashMap<String, Vec<String>>> {
    ...
    let set_level_2_data = |data: &mut Vec<Vec<(String, Vec<String>)>>, point: usize, index: usize| {
        if let Some(vec) = data.last_mut() {
            if let Some(map) = vec.last_mut() {
                let word = string.chars().skip(point + 1).take(index - point - 1).collect();
                map.1.push(word);
            }
        }
    };
    ...
    for d in data {
        ...
        for mut city_connect in d {
            map.entry(city_connect.0).or_insert(Vec::new()).append(&mut city_connect.1);
        }
        result.push(map);
    }
    result
